...
namespace TokenizerNet.Core.Utils
{
    public static class SymbolsLibraryParser
    {
        private const string CommentLine = "#";

        public static IEnumerable<Symbol> ParseFile(string filePath) =>
            ParseInternal(File.ReadAllLines(filePath));

        public static IEnumerable<Symbol> ParseContent(string fileContent) =>
            ParseInternal(fileContent.Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries));

        private static IEnumerable<Symbol> ParseInternal(IList<string> lines)
        {
            foreach (var line in lines.Where(IsDataLine))
            {
